...
- 状态：已接受
- 日期：2026-09-13
- 最后修订：2026-09-16（TASK-030 收口：模型设置界面化——新增 `main/settings/` 与设置面板；陪练点 `buildRuntimeEnv` 已由主人填完）
...
- `main/settings/`（model-settings.ts、settings-ipc.ts、error-code.ts）——TASK-030 的模型配置界面化。指导书没规划「设置」这一层：V0.1 的模型配置本来只有环境变量一条通道，打包版用户没有入口。拆成三块：model-settings 管存储与 env 合成（文件读写与密钥库端口都注入，因此不 import electron 也能单测「加密不可用」「密文解不开」这些分支）、settings-ipc 是 IPC 边界的入参收窄与「先落盘、后重启」编排（与 permission-ipc.ts 同一种纯函数 + 注入 deps 的写法）、error-code 是设置通道的错误码登记表（并进 `shared/ipc-contract.ts` 的 `IpcErrorCode` 联合）。配置仍从 env 通道进子进程——`startRuntime` 显式传 `env`（`buildRuntimeEnv(process.env, settings)`），所以 packages/protocol 与 Python 侧零改动，三步同步不用动。
- `renderer/src/components/SettingsDialog.tsx`——TASK-030 的设置面板（Sidebar 里的入口在 TASK-029 前就预留了占位）。仿 DiagnosticsDialog：随开合挂载、打开时读一次现状；Key 输入框永远从空开始（明文不回传，留空 = 不改），model / baseUrl 留空即清空。
...
- TASK-030 给模型配置开了界面入口，且**没有动协议**：配置仍从 env 通道进子进程（`startRuntime` 显式传 `env = buildRuntimeEnv(process.env, settings)`），所以 `packages/protocol` 与 Python 侧零改动、三步同步不用碰。四处取舍：① 存储用 `userData/model-settings.json` 而不是 product-state 库——设置不是产品状态、没有迁移需求，Key 要的是「文件 + 系统密钥库」而不是 SQL 行；② Key 走 `safeStorage`（Windows 上 DPAPI）加密后存 base64，明文不回传 Renderer（SEC-008：get 通道只回 `apiKeySet` 布尔）、不进日志与错误消息；③ 面板里存过的字段压过同名环境变量（WYSIWYG），但继承环境里有 `PERSONAL_AGENT_SCRIPT` 时三个模型字段一个都不注入——剧本模式是 per-launch 的显式指定（演示脚本、CI、E2E），否则演示会被之前存过的设置静默改成真模型；④ 保存后空闲时自动重启 runtime（Python 启动时读一次 env），用 `PythonSupervisor.busy`（pending 请求数 > 0）挡住「任务跑到一半被换掉」，有任务在跑就返回 `on-next-restart`，界面文案与「已重启」分开说。**陪练点一处**：`buildRuntimeEnv`（思维与算法）——主人一次填对结构与优先级，唯一差别是纯空白字段用真值判断 `if (settings.model)` 会漏过 `'  '`（而他自己写的 SCRIPT 分支已在用 trim 口径），改三行 `?.trim()` 即过；AI 侧把契约里的「空串」补成「空串 / 纯空白」，规格口径没写到底层语义是这一轮唯一的返工。门禁：`pnpm verify` 947 + 122 + 289 全绿。
...
- TASK-031 把「固定五步」管线改成对话式 agent 的第一半（按目标规划 + 自然语言回复），安全链一行没动：计划仍是执行前的合约，ActionAlignment 与交付物判定继续按计划推导，变的只是计划的生产者——`planning.py` 的常量表退居 `planner.py` 端口后的 `DeterministicPlanner`（剧本/CI/演示专用，CON-006 不变），配了 `OPENAI_MODEL` 时由 `live_planner.py` 的 `LivePlanner` 按目标出计划。四处取舍：① 模型给的计划要过 `clean_plan` 清洗，能力必须落在可见清单内，违规**整份作废**（新码 `PLAN_MODEL_FAILED`）而不是静默删步——删一步等于悄悄降低「这次要做什么」的要求，任务会「成功」但没干用户要的事；② 剧本不参与规划：`ScriptedModel` 是带游标的决策序列，让规划去读会先啃掉本该留给循环的那几步；③ 契约三处同步：`RunTaskParams` 增 `plan`（Main 把 make_plan 的回包原样发回，Alignment 与模型提示词用同一份）、`completed` 增 `reply`（自然语言回复，进 task_completed 事件）、`SummaryDecision.reply` 必填；④ 摘要校验按计划分档（`verify_summary` 的 `require_page_refs`）：计划含 extract_pdf 才强制页码，零工具轮次 facts 可空但反编造底线两档共用——TS 侧 `summary_present` / `timeline_evidence` 同步条件化并新增 `reply_present`，取证侧「没有成功的提取」只在计划承诺了提取时才算 gap，否则 fail-closed 总判定会把纯对话轮次无条件卡死。`INSTRUCTIONS` 不再写死五步（稳定规则归提示词，「这轮做什么」归输入里的本轮计划）；`MAKE_PLAN_TIMEOUT_MS` 10s→60s（规划在真模型模式下是一次网络调用）。过程教训两条进测试：中文标点字符串要整段粘贴（render_input 的全角括号手抄成半角，被既有钉值测试逮住）；钉值过时要改钉不要拆网（规划超时改钉 60s 后仍守住「必须比 run_task 短」）。门禁：`pnpm verify` 129 + 963 + 322 全绿；真模型下的按目标规划/纯对话直接回待主人冒烟。
- TASK-032 补上对话式的另一半（多轮会话），安全链仍然一行没动：conversation = 轮次序列，一轮 = 一个 Task（单槽串行、状态机、事件批量落库原样保留）。四处取舍：① 存储分三件——conversations/messages 两张新表（migration 0009）+ tasks 可空 `conversation_id`；任务与会话的关联以 **messages.task_id** 为准（getConversation 按它投影出内嵌 timeline 的消息视图），tasks.conversation_id 留给诊断查询、当前不写；② history 是**可选**契约（第一轮没有），role 只有 user/assistant（system 由提示词承担，SEC-006 口径不变），随 make_plan 与 run_task 双通道下发——LivePlanner 靠它解析「它」「那个文件」，engine 把它渲染进执行提示词；确定性规划器刻意忽略（CON-006）；③ 历史裁剪在 TS 侧（`buildHistory`：从新到旧、条数+字符两级预算、历史必须以 user 开头、修头只丢不补），Python 不重复预算、只对收到的每串文本走 2000 字符截断；④ 纯对话轮次（计划无 extract_pdf）facts 可空、以 reply 承载回答：闸口新增 `reply_present`，「没有成功的提取」缺口按计划分档——fail-closed 总判定不再无条件卡死零工具轮次。UI 层换成「会话 + 消息数组」（TanStack Query 管会话列表缓存），渲染层的 run-task 通道退役（主进程函数保留给 eval/e2e）。真机冒烟逮住的 bug：App 把 invalidateQueries 包成了 useMutation，而 mutationFn 是真实的第二次 sendMessage——每轮对话被完整执行两遍；缓存失效要直接调 invalidate，不能包成会执行的 mutation。新增会话 E2E（两轮同会话：第二轮 RPC 入参带第一轮历史、消息与任务同会话、getConversation 投影）。门禁：`pnpm verify` 135 + 995 + 333 全绿。