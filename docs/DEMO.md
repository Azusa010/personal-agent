...
**TASK-032 起是多轮对话**：每条消息进入一个会话（首条消息创建会话并成为标题），
追问自动追加到当前会话，agent 随请求携带此前轮次作为上下文。两种模式下多轮的表现
不同：剧本模式（A）每一轮重放同一份剧本，回复文本相同——确定性优先；真模型模式
（B）规划器按目标出计划，纯问候/追问可以零工具直接回答，多轮的上下文才会真正
参与规划与执行。验收多轮请用路径 B。
