...
describe('idempotencyKey：内容派生的稳定键', () => {
  ...
  it('格式是 capability:argsHash', () => {
    expect(idempotencyKey('filesystem.move', bound)).toBe(
      `filesystem.move:${fingerprintArguments(bound).hash}`
    )
  })

  it('同能力同参数 → 同 key（跨调用稳定）', () => {
    expect(idempotencyKey('filesystem.move', bound)).toBe(idempotencyKey('filesystem.move', bound))
  })

  it('不同能力 → 不同 key（参数相同也区分）', () => {
    expect(idempotencyKey('filesystem.move', bound)).not.toBe(
      idempotencyKey('filesystem.create_dir', bound)
    )
  })
  ...
  it('不同参数 → 不同 key', () => {
    ...
    expect(idempotencyKey('filesystem.move', bound)).not.toBe(
      idempotencyKey('filesystem.move', other)
    )
  })
})
