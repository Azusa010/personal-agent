...
const SEQUENCE = ['filesystem_list', 'document_extract_pdf']

describe('checkAlignment：按计划顺序放行', () => {
  it('第 1 次调用对得上第 1 个带 capability 的步骤', () => {
    expect(checkAlignment(PLAN, 0, 'filesystem_list')).toEqual({ aligned: true })
  })

  it('第 2 次调用对得上第 2 个带 capability 的步骤', () => {
    expect(checkAlignment(PLAN, 1, 'document_extract_pdf')).toEqual({ aligned: true })
  })
  ...
  it('单步计划：第 1 次放行，第 2 次超出', () => {
    const single: readonly PlanStep[] = [
      { description: '只列一次', capability: 'filesystem_list' },
      { description: '收尾，不调工具' }
    ]

    expect(checkAlignment(single, 0, 'filesystem_list')).toEqual({ aligned: true })
    expect(checkAlignment(single, 1, 'filesystem_list').aligned).toBe(false)
  })
})
...
describe('checkAlignment：跑偏', () => {
  ...
  it('第 2 次重复调第一个能力 -> not aligned', () => {
    const out = checkAlignment(PLAN, 1, 'filesystem_list')

    expect(!out.aligned && out.reason).toBe(
      '第 2 次 tool call 期望 document_extract_pdf，实际是 filesystem_list'
    )
  })
  ...
  it('计划里的能力用完之后再来一次 -> not aligned，reason 说清计划几步', () => {
    ...
    const out = checkAlignment(PLAN, 2, 'filesystem_list')
  ...
  it('远超计划长度同样是超出，序号跟着 executedCalls 走', () => {
    const out = checkAlignment(PLAN, 7, 'filesystem_list')
  ...
  it('计划里一步 capability 都没有 -> 任何调用都超出', () => {
    ...
    const out = checkAlignment(planless, 0, 'filesystem_list')
  ...
  it('空计划 -> not aligned，而不是放行', () => {
    ...
    expect(checkAlignment([], 0, 'filesystem_list').aligned).toBe(false)
  })
})
...
describe('checkAlignment：纯函数', () => {
  it('不修改 plan', () => {
    ...
    checkAlignment(PLAN, 0, 'filesystem_list')
    checkAlignment(PLAN, 5, 'document_extract_pdf')
  ...
  it('同一个输入两次调用结果相同', () => {
    ...
    const first = checkAlignment(PLAN, 1, 'filesystem_list')
