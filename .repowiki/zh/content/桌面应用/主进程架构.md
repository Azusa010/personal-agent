...
章节来源
- [apps/desktop/src/main/index.ts:268-318](file://apps/desktop/src/main/index.ts#L268-L318)
- [apps/desktop/src/main/runtime/runtime-host.ts:92-214](file://apps/desktop/src/main/runtime/runtime-host.ts#L92-L214)
- [apps/desktop/src/main/runtime/python-supervisor.ts:66-160](file://apps/desktop/src/main/runtime/python-supervisor.ts#L66-L160)
- [apps/desktop/src/main/capabilities/host-executor.ts:62-117](file://apps/desktop/src/main/capabilities/host-executor.ts#L62-L117)
...
章节来源
- [apps/desktop/src/main/index.ts:109-377](file://apps/desktop/src/main/index.ts#L109-L377)
...
图表来源
- [apps/desktop/src/preload/index.ts:18-20](file://apps/desktop/src/preload/index.ts#L18-L20)
...
- 启动流程：
  - 解析运行时命令与工作目录（`resolveRuntimeLaunch` 纯函数，按 `PERSONAL_AGENT_RUNTIME` 覆盖 > 打包布局 > 开发布局解析；TASK-029）
  - 检查运行时命令是否存在（按布局解析：PERSONAL_AGENT_RUNTIME 覆盖 / 打包冻结产物 / 仓库 venv），不存在则标记 crashed
  - 创建 PythonSupervisor，注入 hostHandler 与可见能力清单
  - 监听 stderr、runtime.crashed 事件
  - 调用 initialize 握手，成功后状态置为 ready
- 布局差异：打包布局的 `args` 为空（冻结产物自带入口，`<安装目录>/resources/agent-runtime/personal_agent.exe`），开发布局是 `.venv/Scripts/python.exe -m personal_agent`
- 停止流程：
...
- 环境注入（TASK-030）：创建 supervisor 时显式传 `env`——`buildRuntimeEnv(process.env, 设置面板里存过的模型配置)` 逐字段合成：已存字段覆盖同名 `OPENAI_*`、未填字段保留继承值、继承环境里有 `PERSONAL_AGENT_SCRIPT` 时三个模型字段都不注入（剧本模式是 per-launch 的显式指定）。
- 设置生效：`restartRuntime()` 空闲时 stop + start（Python 启动时读一次 `OPENAI_MODEL`，改配置只能靠重启）；`PythonSupervisor.busy`（未决请求数 > 0）挡住「任务跑到一半被换掉」，有任务在跑就把生效推到任务结束。
...
- [apps/desktop/src/main/runtime/runtime-host.ts:24-202](file://apps/desktop/src/main/runtime/runtime-host.ts#L24-L202)
- [apps/desktop/src/main/runtime/python-supervisor.ts:96-160](file://apps/desktop/src/main/runtime/python-supervisor.ts#L96-L160)
- [apps/desktop/src/main/runtime/runtime-host.ts:153-161](file://apps/desktop/src/main/runtime/runtime-host.ts#L153-L161)
- [apps/desktop/src/main/runtime/runtime-host.ts:185-190](file://apps/desktop/src/main/runtime/runtime-host.ts#L185-L190)
- [apps/desktop/src/main/settings/model-settings.ts:199-224](file://apps/desktop/src/main/settings/model-settings.ts#L199-L224)
...
## 故障排查指南
- Runtime 未启动或崩溃
  - 检查运行时命令是否存在（按布局解析：PERSONAL_AGENT_RUNTIME 覆盖 / 打包冻结产物 / 仓库 venv）；查看 stderr 输出；确认 initialize 握手是否成功
...
- [apps/desktop/src/main/runtime/python-supervisor.ts:232-262](file://apps/desktop/src/main/runtime/python-supervisor.ts#L232-L262)
