...
- 关键点
  - 跳过条件：若仓库 venv（或冻结产物）不存在则整块跳过，保证 CI 稳定。
...
**章节来源**
- [apps/desktop/src/main/e2e/failure-regression.test.ts:1-479](file://apps/desktop/src/main/e2e/failure-regression.test.ts#L1-L479)
- [apps/desktop/src/main/e2e/fault-expectations.ts:1-216](file://apps/desktop/src/main/e2e/fault-expectations.ts#L1-L216)
...
### 打包冒烟（packaged-runtime，TASK-029）
- 目标：验证「随包分发的那个 exe 还是不是一个完整运行时」——握手、剧本加载、反向 RPC、摘要校验、进程退出。真启动安装包、点按钮、看窗口那条留给 docs/DEMO.md 的人工清单。
- 跳过条件：冻结产物 `services/agent-runtime/dist/personal_agent/` 不存在则整块跳过（先 `pnpm package:py`，再 `pnpm test:ts` 才会真的跑）。
- 四条用例：
  - 握手拿到与源码运行时一致的服务器信息；
  - 剧本按五步走完、每步经反向 RPC 到达 host（READ 回真形状结果、WRITE 一律拒绝），摘要过校验后 completed——同时证明冻结产物里 SummaryVerifier 也在工作；
  - stop 之后子进程真的退出，不留孤儿（TEST-015 的退出条件）；
  - live 模式的 SDK 随包可用：假地址（127.0.0.1:1，带 NO_PROXY 直连）下收成 MODEL_CALL_FAILED，不是进程崩溃。
- 与 `e2e/golden-path.test.ts` 的分工：那边用 venv 跑真链路，覆盖「行为对不对」；这边用冻结产物跑关键节点，覆盖「打包形态能不能跑」。

**章节来源**
- [apps/desktop/src/main/runtime/packaged-runtime.test.ts:1-227](file://apps/desktop/src/main/runtime/packaged-runtime.test.ts#L1-L227)
- [apps/desktop/src/main/runtime/runtime-host.test.ts:1-81](file://apps/desktop/src/main/runtime/runtime-host.test.ts#L1-L81)
