...
#### 前置条件
- 操作系统：Windows 10/11（V0.1 只支持 Windows，CON-003）
- 已安装 Node.js（建议 LTS）
- 已安装 Python 3.13+（推荐通过 pyenv 或系统包管理器安装；只有开发与打包需要，安装包本身不要求目标机器装 Python）
...
- 使用 uv 安装依赖（推荐 --locked 以锁定 uv.lock）：
  - uv sync --locked
  - 说明：pyproject.toml 声明了 openai、pydantic 等依赖；dev 组包含 pytest、pytest-asyncio、ruff 与 pyinstaller（冻结打包用，TASK-029）
...
- [services/agent-runtime/.python-version:1-2](file://services/agent-runtime/.python-version#L1-L2)
- [services/agent-runtime/uv.lock:1-475](file://services/agent-runtime/uv.lock#L1-L475)
...
- 打包（Windows，两步，顺序不能反）：
  - pnpm package:py（PyInstaller 冻结 Python 运行时）
  - pnpm package:dir（免安装目录）/ pnpm package:win（NSIS 安装包）——两者内部都先跑 package:py 再交给 electron-builder
  - 一步到位的演示：powershell -File scripts/demo/start-demo.ps1（见 docs/DEMO.md）
...
- 运行时位置覆盖（TASK-029）：PERSONAL_AGENT_RUNTIME 指向一个自带入口的可执行文件，整体替换「打包布局 / 开发布局」的解析结果，供冒烟与排障使用。
...
- [services/agent-runtime/.python-version:1-2](file://services/agent-runtime/.python-version#L1-L2)
- [services/agent-runtime/pyproject.toml:1-27](file://services/agent-runtime/pyproject.toml#L1-L27)
- [services/agent-runtime/uv.lock:1-235](file://services/agent-runtime/uv.lock#L1-L235)
