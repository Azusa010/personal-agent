...
- 桌面端
  ...
  - 平台打包：package:dir / package:win（Windows；内含 package:py）
- Python
  - Lint：pnpm lint:py
  - 修复：pnpm fix:py
  - 测试：pnpm test:py
...
- [services/agent-runtime/pyproject.toml:17-26](file://services/agent-runtime/pyproject.toml#L17-L26)
...
**图表来源**
- [AGENTS.md:213-227](file://AGENTS.md#L213-L227)
- [apps/desktop/electron.vite.config.ts:6-23](file://apps/desktop/electron.vite.config.ts#L6-L23)
...
- [apps/desktop/package.json:17-18](file://apps/desktop/package.json#L17-L18)
...
- 平台打包：
  ...
  - V0.1 只出 Windows 产物（CON-003）：package:dir（免安装目录）/ package:win（NSIS 安装包）
...
- [apps/desktop/package.json:18-22](file://apps/desktop/package.json#L18-L22)
...
- 分工：AI 完整写出协议 schema/fixtures、类型与接口、migration DDL、仓储 SQL 样板、import 与接线、UI 布局，以及测试的**场景与数据**（seed / 用例标题 / 期望行为的文字描述）；核心业务逻辑本体留给主人填写。
- 留什么的判据是**五类陪练点**（AGENTS.md §6）：思维与算法（核心计算、数据转换、业务主逻辑——状态机转换表、判定/校验函数、算法段）、边界与异常（异常捕获与分流、超时降级、兜底、并发锁）、工程与规范（日志打点、硬编码抽离、配置解耦）、架构与设计（策略/表驱动重构点、N+1 与串行 await 一类性能隐患）、验证与质量（单元测试的**断言**部分）。
- 标记统一为 `// TODO(你填)[类别]: ...`（Python 侧 `# TODO(你填)[类别]: ...`），全仓搜索该标记即为待办清单；填完必须删掉标记，保证搜索不出现陈旧项。
- 每个 TODO 必须自带足够上下文：输入输出契约、不变量与边界条件、失败时的稳定错误码、对应验收测试的文件名。
- 断言留给主人的用例：AI 保留场景与 seed，函数调用照留（占位实现会让它红），`expect(...)` 换成「期望：<文字描述>」；**契约底线断言除外**（报告结构、fail-closed 底线、安全不变量）——那些由 AI 保留，不随陪练交出去。
- AI 写脚手架时不给数据记录加 `readonly`：留给主人构造/回填的类型字段一律可变，只读数组也别加——TS 的 readonly 只挡构造、不提供运行时保护，加了它等于把「先建空对象再回填」这种最直白的写法堵死。
- AI 不得顺手代填 TODO 范围内的核心逻辑；主人填完前相关测试允许红，填完后 pnpm verify 必须全绿。
- 验收流程：主人填 TODO → AI 跑 pnpm verify → 对照指导书该 TASK 的 Validation 列核对 → 通过后标 ✅ 并提交。主人明确授权「你来写」时，AI 可直接填完并把标记清零（须记入 ADR）。
- 归属边界拿不准时，按「这段代码错了会不会造成副作用/安全问题/契约漂移」判断：会 → 留给主人；不会 → AI 直接写完。
- 先例：product-state/tool-execution-repository.ts 的 `ALLOWED_TRANSITIONS` 转换表是第一个陪练点（原 TODO 标记已随填写移除），逐格一致性断言的测试由 AI 预先写好；main/verification 是五类各留一处的完整样本，标记已随填写移除，要回看形状翻 git 历史。

**章节来源**
- [AGENTS.md:188-227](file://AGENTS.md#L188-L227)
...
**章节来源**
- [AGENTS.md:122-145](file://AGENTS.md#L122-L145)
- [apps/desktop/electron-builder.yml:7-13](file://apps/desktop/electron-builder.yml#L7-L13)
...
- 日常开发
  ...
  - 测试：pnpm test
  - 构建：pnpm build
  - 打包：pnpm package:dir / package:win
- 提交前校验
  - 全量校验：pnpm verify
  - 全量校验+构建：pnpm verify:all
- 陪练待办（TODO(你填)）
  - 全仓搜索 `TODO(你填)` 标记即为待填清单，标记带类别（思维与算法 / 边界与异常 / 工程与规范 / 架构与设计 / 验证与质量），可按维度集中练
