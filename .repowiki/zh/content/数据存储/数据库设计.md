...
- reminders：提醒表（migration 0007），id 主键，task_id 外键关联 tasks(id) 且带 UNIQUE——「同一 Task 至多一条 Reminder」由数据库级保证。status 受 CHECK 约束限定为 scheduled/firing/fired/failed；索引 idx_reminders_due(status, remind_at) 供到期扫描（启动恢复 TASK-025 规划使用，属预留索引）。idempotency_key 故意不设 UNIQUE：key = capability:argsHash 不含 taskId，两个不同任务参数恰好相同时 key 相等，但那不算重复 Reminder。
...
- 索引与查询
  ...
  - reminders 建立 idx_reminders_due(status, remind_at) 复合索引，供到期扫描使用（启动恢复 TASK-025 规划中，当前为预留索引）。
- 事务与批处理
