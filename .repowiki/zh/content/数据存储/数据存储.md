...
- [apps/desktop/src/main/product-state/migrations/index.ts:13-22](file://apps/desktop/src/main/product-state/migrations/index.ts#L13-L22)
...
### 模型设置文件（userData/model-settings.json，TASK-030）
- 唯一一类不进 SQLite 的持久状态：模型 API Key、模型名与 API Base URL。
- Key 以 safeStorage（Windows 上 DPAPI）密文存 base64，明文不落盘（SEC-008）；model / baseUrl 明文。
- 读取失败（缺文件 / 版本号不认识 / 字段类型不对 / 密文解不开）整份按「未配置」处理——手改坏文件与换 Windows 账户都属于这一类：不抛异常、不挡 app 启动，也不逐字段降级（避免出现「面板有值、运行时用残缺配置」的组合）。
- 写盘前把空串与纯空白归一成 null；版本号不符不做迁移，宁可让用户重填。

章节来源
- [model-settings.ts:9-13](file://apps/desktop/src/main/settings/model-settings.ts#L9-L13)
- [model-settings.ts:42-48](file://apps/desktop/src/main/settings/model-settings.ts#L42-L48)
- [model-settings.ts:102-137](file://apps/desktop/src/main/settings/model-settings.ts#L102-L137)
- [model-settings.ts:144-175](file://apps/desktop/src/main/settings/model-settings.ts#L144-L175)
...
- 迁移
  - 迁移入口：参考 [apps/desktop/src/main/product-state/migrations/index.ts:13-22](file://apps/desktop/src/main/product-state/migrations/index.ts#L13-L22)
...
- [apps/desktop/src/main/product-state/permission-repository.ts:83-93](file://apps/desktop/src/main/product-state/permission-repository.ts#L83-L93)
- [apps/desktop/src/main/db/pdf-repository.ts:15-30](file://apps/desktop/src/main/db/pdf-repository.ts#L15-L30)
