...
- [apps/desktop/src/main/product-state/migrations/index.ts:1-23](file://apps/desktop/src/main/product-state/migrations/index.ts#L1-L23)

## 核心组件
- 迁移接口与注册表
  - Migration 接口：包含 version、name、up(db)。
  - MIGRATIONS 数组：按版本号升序注册全部迁移，当前共 8 条（0001~0008）。0005 与 0008 都是「改约束」型迁移（SQLite 改不了已有约束，只能重建表）：0005 放宽 tasks.status 的 CHECK，0008 把 permissions 的 tool_call_id UNIQUE 收窄成 UNIQUE(task_id, tool_call_id)。
- 迁移引擎 migrate
  - 去重校验：迁移版本必须唯一。
...
- 0007：创建 reminders 表（Reminder 持久化），id 主键、task_id UNIQUE + 外键、status CHECK（scheduled/firing/fired/failed），并建立 idx_reminders_due(status, remind_at) 到期扫描索引（当前恢复走 findAll 全表扫描，该索引仍为预留）。
...
- 索引优化
  ...
  - reminders 针对 (status, remind_at) 建立索引（idx_reminders_due），供到期扫描；启动恢复已实现但走 findAll 全表扫描，该索引尚未被使用。
- 并发注意事项
