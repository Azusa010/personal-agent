...
- [App.tsx:234-300](file://apps/desktop/src/renderer/src/App.tsx#L234-L300)
...
### SettingsDialog：模型设置
- 功能
  - 侧栏「设置」入口打开的模型配置面板：API Key、模型名、API Base URL 三项。
  - Key 输入框永远从空开始（明文不回传 Renderer，SEC-008），留空 = 不改；模型名与 Base URL 留空 = 清空。
  - 保存后按结局显示文案：runtime 已重启（新配置立即生效），或有任务在跑（任务结束后重启生效）。
  - 已配 Key 时出现「清除已存的 Key」按钮，走 clearApiKey 显式清除，不做隐式清空。
- 可访问性
  - 每个输入框配 Label；Key 用 type=password、autoComplete=off。

章节来源
- [SettingsDialog.tsx:1-205](file://apps/desktop/src/renderer/src/components/SettingsDialog.tsx#L1-L205)
- [App.tsx:294-298](file://apps/desktop/src/renderer/src/App.tsx#L294-L298)
...
章节来源
- [App.tsx:48-115](file://apps/desktop/src/renderer/src/App.tsx#L48-L115)
