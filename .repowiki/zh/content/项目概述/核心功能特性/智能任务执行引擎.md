...
6) 引擎产出事件与最终结果（completed/failed），主进程将事件与任务状态落库。
...
- 事务 B：批量写入事件、更新任务状态为 completed/failed。
...
### 任务生命周期管理
- 状态机：pending → running → completed/failed/cancelled/waiting_permission。
...
- [engine.py:67-74](file://services/agent-runtime/src/personal_agent/engine.py#L67-L74)
...
- [python-supervisor.ts:197-208](file://apps/desktop/src/main/runtime/python-supervisor.ts#L197-L208)
...
- 发起任务执行
  ...
  - 参考路径：[run-task.ts:86-223](file://apps/desktop/src/main/tasks/run-task.ts#L86-L223)
- 处理异步响应
