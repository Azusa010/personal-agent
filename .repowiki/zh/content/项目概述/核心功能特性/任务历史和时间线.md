...
- 任务状态机
  ...
  - completed 的触发者是交付物闸口（TASK-026）：run-task 落完事件后状态仍留在 running，判定表通过才由事务 B2 翻 completed；拒绝时写 verification_failed + task_failed(RUNTIME_VERIFICATION_FAILED) 并落 failed。时间线因此多出三条 TS 侧事件：verification_started / verification_passed / verification_failed，payload 里带判定报告与 Evidence Bundle。
- 事件模型
