...
图表来源
- [apps/desktop/src/main/runtime/python-supervisor.ts:96-133](file://apps/desktop/src/main/runtime/python-supervisor.ts#L96-L133)
...
章节来源
- [apps/desktop/src/main/runtime/python-supervisor.ts:96-363](file://apps/desktop/src/main/runtime/python-supervisor.ts#L96-L363)
...
章节来源
- [apps/desktop/src/main/runtime/python-supervisor.ts:114-132](file://apps/desktop/src/main/runtime/python-supervisor.ts#L114-L132)
...
章节来源
- [apps/desktop/src/main/runtime/python-supervisor.ts:209-229](file://apps/desktop/src/main/runtime/python-supervisor.ts#L209-L229)
- [apps/desktop/src/main/runtime/python-supervisor.ts:339-363](file://apps/desktop/src/main/runtime/python-supervisor.ts#L339-L363)
...
- 案例一：子进程崩溃导致所有请求失败
  ...
  - 步骤：
    ...
    3) 重启应用并复现，收集日志。
  - 参考实现
    - [apps/desktop/src/main/runtime/python-supervisor.ts:127-132](file://apps/desktop/src/main/runtime/python-supervisor.ts#L127-L132)
...
- 打包与运行时定位速查（TASK-029）：
  - 「找不到运行时（打包布局）」：`<安装目录>/resources/agent-runtime/personal_agent.exe` 缺失——单独跑 `build:win` 会漏掉 `package:py`，先跑 `pnpm package:dir`。
  - electron-builder 日志出现 `file source doesn't exist`：`extraResources.from` 相对 apps/desktop 写错了，产物会少一个目录且不报错。
  - 演示脚本的剧本读不了：BOM 问题（PowerShell 5.1 的 `-Encoding UTF8` 会写 BOM），用 `UTF8Encoding($false)` 写盘。
  - 完整演示排障清单见 docs/DEMO.md。
