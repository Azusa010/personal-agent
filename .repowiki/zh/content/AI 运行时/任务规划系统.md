...
## 简介
本系统为 PersonalAgent 的任务规划子系统，负责将用户目标转化为可执行的步骤计划，并在执行阶段对模型调用进行能力对齐与权限控制。Phase 2 的计划是“固定三步”的确定性流程：先列出 PDF，再提取每页文本，最后由模型生成带页码引用的摘要。该设计确保跨语言（TS/Python）契约一致、行为可复现、便于端到端测试与回归。
...
- [scope.ts:1-18](file://apps/desktop/src/main/capabilities/scope.ts#L1-L18)
...
## 核心组件
- 计划生成：make_plan 根据可见能力清单输出固定三步计划；若缺少必需能力则抛出 PlanError。
...
### make_plan 实现原理
- 目标分解算法：当前为“固定三步”模板，不随 goal 变化而改变顺序或数量，保证可复现性与对齐基准稳定。
- 步骤生成逻辑：
...
- [runtime.py:37-40](file://services/agent-runtime/src/personal_agent/runtime.py#L37-L40)
- [error-code.ts:1-17](file://apps/desktop/src/main/runtime/error-code.ts#L1-L17)
...
- [runtime.py:37-40](file://services/agent-runtime/src/personal_agent/runtime.py#L37-L40)
- [error-code.ts:1-17](file://apps/desktop/src/main/runtime/error-code.ts#L1-L17)
- [test_runtime.py:555-604](file://services/agent-runtime/tests/test_runtime.py#L555-L604)
...
图表来源
- [runtime.py:1-28](file://services/agent-runtime/src/personal_agent/runtime.py#L1-L28)
- [planning.py:1-23](file://services/agent-runtime/src/personal_agent/planning.py#L1-L23)
- [engine.py:1-41](file://services/agent-runtime/src/personal_agent/engine.py#L1-L41)
...
## 结论
本规划系统在 Phase 2 采用“固定三步”的确定性计划，结合严格的协议约束与能力可见性控制，确保执行过程安全、可审计、可复现。通过 alignment 策略与预算控制，系统在灵活性与安全性之间取得平衡。未来可扩展为动态规划，但仍需保持契约一致与可测试性。
