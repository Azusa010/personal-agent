...
图表来源
- [run-task.ts:86-223](file://apps/desktop/src/main/tasks/run-task.ts#L86-L223)
- [python-supervisor.ts:66-342](file://apps/desktop/src/main/runtime/python-supervisor.ts#L66-L342)
...
Sup-->>Main : 返回结果
Main->>DB : 事务B : 写入所有事件/更新任务状态
Main-->>UI : 返回最终结果
```

图表来源
- [run-task.ts:86-223](file://apps/desktop/src/main/tasks/run-task.ts#L86-L223)
- [python-supervisor.ts:123-194](file://apps/desktop/src/main/runtime/python-supervisor.ts#L123-L194)
...
图表来源
- [engine.py:89-147](file://services/agent-runtime/src/personal_agent/engine.py#L89-L147)
- [engine.py:148-166](file://services/agent-runtime/src/personal_agent/engine.py#L148-L166)
...
- 依赖管理：
  - 计划由 planning.py 固定三步，依赖 filesystem.list 与 document.extract_pdf 能力。
...
Main->>DB : 事务B : append(events), updateStatus(status)
DB-->>Main : 确认
Main-->>UI : 返回最终结果
```
...
- [python-supervisor.ts:271-299](file://apps/desktop/src/main/runtime/python-supervisor.ts#L271-L299)
...
章节来源
- [run-task.ts:25-37](file://apps/desktop/src/main/tasks/run-task.ts#L25-L37)
- [run-task.ts:182-223](file://apps/desktop/src/main/tasks/run-task.ts#L182-L223)
