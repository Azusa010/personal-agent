...
### 配置来源与优先级：设置面板、环境变量与剧本（TASK-030）
- 子进程拿到的模型配置有两个来源，由 Electron 主进程在 spawn 时合成：
  - 设置面板（userData/model-settings.json，Key 用系统密钥库加密）里填过的字段；
  - 继承的 Electron 进程环境变量（开发态 shell 里 export 的那套；CI、E2E 与打包冒烟走这里）。
- 合成规则（buildRuntimeEnv，纯函数）：
  - 已存字段覆盖同名 OPENAI_*；未填字段保留继承值。
  - 继承环境里有非空 PERSONAL_AGENT_SCRIPT 时三个模型字段一个都不注入——剧本模式是 per-launch 的显式指定（演示脚本、CI、E2E），压过持久设置，否则演示会被面板里存过的设置静默改成真模型。
  - 空串与纯空白按「未设」处理：Python 侧 os.environ.get 拿到 '  ' 仍是真值，而存储层写盘前已把纯空白归一成 null。
- 生效时机：Python 在进程启动时读一次 OPENAI_MODEL（resolve_model_factory），所以面板保存后由主进程重启 runtime 才生效；有任务在跑则推迟到任务结束（PythonSupervisor.busy = 未决请求数 > 0）。

```mermaid
flowchart TD
A["settings.load()：面板存过的字段"] --> C{"buildRuntimeEnv(process.env, settings)"}
B["process.env：shell / 演示脚本"] --> C
C --> |"有非空 PERSONAL_AGENT_SCRIPT"| D["不注入三个 OPENAI_*（剧本优先）"]
C --> |"否则逐字段覆盖，空串/纯空白不注入"| E["子进程 env"]
D --> E
E --> F["runtime.py 启动时读 OPENAI_MODEL / SDK 读 OPENAI_API_KEY"]
```

**图表来源**
- [model-settings.ts:199-224](file://apps/desktop/src/main/settings/model-settings.ts#L199-L224)
- [runtime-host.ts:153-161](file://apps/desktop/src/main/runtime/runtime-host.ts#L153-L161)
- [python-supervisor.ts:106-110](file://apps/desktop/src/main/runtime/python-supervisor.ts#L106-L110)

**章节来源**
- [model-settings.ts:199-224](file://apps/desktop/src/main/settings/model-settings.ts#L199-L224)
- [runtime-host.ts:114-134](file://apps/desktop/src/main/runtime/runtime-host.ts#L114-L134)
- [runtime-host.ts:185-190](file://apps/desktop/src/main/runtime/runtime-host.ts#L185-L190)
- [runtime.py:208-235](file://services/agent-runtime/src/personal_agent/runtime.py#L208-L235)
...
- [context.py:9-12](file://services/agent-runtime/src/personal_agent/context.py#L9-L12)
- [host_channel.py:8-13](file://services/agent-runtime/src/personal_agent/host_channel.py#L8-L13)
- [uv.lock:171-186](file://services/agent-runtime/uv.lock#L171-L186)
...
- 参考路径：
  ...
  - 依赖项：[uv.lock:171-186](file://services/agent-runtime/uv.lock#L171-L186)
