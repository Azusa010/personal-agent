...
- 计划生成器：根据目标与可见能力清单输出确定性计划（READ 必给 / WRITE 可见才进 / 摘要最后；完整配置六步，只读配置三步）。
...
章节来源
- [runtime.py:77-80](file://services/agent-runtime/src/personal_agent/runtime.py#L77-L80)
...
- 计划逻辑：确定性、随可见能力伸缩（完整六步 / 只读三步），要求 filesystem.list 与 document.extract_pdf 在可见能力中；三个 WRITE 可见才进计划。
...
- [engine.py:213-244](file://services/agent-runtime/src/personal_agent/engine.py#L213-L244)
