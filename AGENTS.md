...
- TASK-031 的 `summary.py` `verify_summary` 分档（边界与异常）：`require_page_refs` keyword-only + 默认 True（分档只放宽不收紧，fail-closed），反编造检查两档共用——零工具轮次页码可选，给了就必须真。**填的经过**：主人一次填对分档结构，三处偏差全被预先落好的测试当场指出——`clean_plan` 与 `_parse` 各一处条件极性写反（None 被当成不可见能力、isinstance 方向反了）、报错序号 `enumerate` 少 `start=1`（「第 2 条」报成「第 1 条」）。本轮起分工收紧为**「AI 只写测试文件与文档，其余实现一律参考代码由主人落盘」**；教训：给实现参考代码时，含中文标点的字符串必须标「整段粘贴」——手抄会把全角括号写成半角，肉眼不可见，只有钉值测试能逮住；钉值过时要改钉不要拆网（规划超时改钉 60s 后仍守住「必须比 run_task 短」）。验收：`pnpm verify` 全绿（129 + 963 + 322）。
- TASK-032 的 `tasks/history.ts` `buildHistory`（TASK-032，边界与异常）：把会话消息裁成有界的 history。契约四条不变量写在函数上方（纯函数 / 从新到旧、条数+字符两级预算 / 历史必须以 user 开头 / 只映射不加工），9 条聚焦用例由 AI 预先写好。**填的经过**：主人第一版把配对规则融进收集循环的最后一个槽位——丢掉孤答之后又把它的提问回填进来，得到 [u1, u2, a2]（一条答案不在历史里的问题）。正确形状是**两个阶段**：先收集（两条预算，放得下就取、放不下就停），再修头（从头丢到 user 开头，只丢不补）；回填等于塞进一个答案已被裁掉的问题，指代解析会更乱。**真机冒烟另逮一 bug**：App 层把 invalidateQueries 包成了 useMutation，而 mutationFn 是真实的第二次 sendMessage——每轮对话被完整执行两遍、会话里出现重复轮次；缓存失效要直接调 invalidate，不能包成会执行的 mutation。验收：`pnpm verify` 全绿（135 + 995 + 333）+ 会话 E2E + 真机多轮冒烟。
